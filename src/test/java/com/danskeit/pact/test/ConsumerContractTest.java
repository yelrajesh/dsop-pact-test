...
import com.danskeit.pact.PactTestService;
...
public class ConsumerContractTest {
	...
	@Rule
	public PactProviderRuleMk2 mockProvider = new PactProviderRuleMk2("bank_provider", "localhost", 8082,
			PactSpecVersion.V2, this);
	
	@DisplayName("Test MessageService.get()")
	@Test
    void testGet() {
        assertEquals("Hello JUnit 5", PactTestService.get());
    }

	private void createRequestHeaders(HttpHeaders accountListHeaders, HttpHeaders emptyAccountListHeaders,
			HttpHeaders wrongAccountListHeaders, HttpHeaders detailsHeaders, HttpHeaders cardsHeaders,
			HttpHeaders rolesHeaders, HttpHeaders transactionHeaders) {
	...
	private void verifyRoles(RestTemplate restTemplate, HttpHeaders accountCommonHeaders) {
		...
		assertThatJson(jsonRoles).when(Option.IGNORING_ARRAY_ORDER)
	...
	private void verifyCards(RestTemplate restTemplate, HttpHeaders accountCommonHeaders) {
		...
		assertThatJson(jsonCards).when(Option.IGNORING_ARRAY_ORDER)
	...
	private void verifyTransactions(RestTemplate restTemplate, HttpHeaders accountCommonHeaders) {
		String transactionsUrl = mockProvider.getUrl()
				+ "/v1/accounts/5687123451/transactions?fromDate=2016-12-09&toDate=2016-12-09";
		...
		assertThatJson(jsonTransactions).when(Option.IGNORING_ARRAY_ORDER)
	...
	private void verifyAccountDetails(RestTemplate restTemplate, HttpHeaders accountListHeaders) {
		String accountDetailsUrl = mockProvider.getUrl()
				+ "/v1/accounts/5687123451?fromDate=2016-12-09&toDate=2016-12-09";
		ResponseEntity<String> accountDetailsResponse = sendRequest(restTemplate, accountListHeaders,
				accountDetailsUrl);
		...
		assertThatJson(jsonAccountDetails).when(Option.IGNORING_ARRAY_ORDER)
	...
	private void verifyAccountList(RestTemplate restTemplate, HttpHeaders accountListHeaders) {
		...
		ResponseEntity<String> accountsResponse = sendRequest(restTemplate, accountListHeaders, accountList);

		String jsonAccounts = accountsResponse.getBody();
		assertThat(accountsResponse.getStatusCode()).isSameAs(HttpStatus.OK);
		assertThatJson(jsonAccounts).when(Option.IGNORING_ARRAY_ORDER)
